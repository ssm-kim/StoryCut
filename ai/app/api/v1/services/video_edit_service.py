...
async def select_time_ranges_by_prompt(video_path: str, user_prompt: str, threshold: float = 0.5):
    logger.info("[행동분석] 분석 시작: %s, threshold=%.2f", video_path, threshold)
    filtered_data = []
    raw_data = await run_analysis_pipeline(video_path)
    logger.info("[행동분석] 분석 완료, 총 %d개 구간", len(raw_data))
    ...
    if not filtered_data:
        logger.info("[행동분석] 기준 이상의 구간 없음 → 원본 복사")
        ...
        logger.info("[행동분석] 복사 완료 → %s", output_path)
        return output_path, raw_data

    try:
        logger.info("[Gemini] Gemini에 프롬프트 전송 중")
        ...
        logger.info("[Gemini] 응답 시간대 수: %d", len(time_ranges))
        ...
        raise RuntimeError(f"Gemini 호출 실패\n응답:\n{response_text if 'response_text' in locals() else '없음'}")

# Gemini에 요청을 보내 필터링된 행동 정보와 프롬프트를 기반으로 관심 시간대를 추출하도록 함
def gemini_translate_ko_to_en(prompt_ko: str, filtered_data: List[Tuple[Tuple[float, float], List[Tuple[str, float]]]]) -> str:
...
def fast_cut_and_merge_ffmpeg(video_path: str, time_ranges: List[Tuple[float, float]]) -> str:
    ...
    logger.info("[편집] 자르기 시작: %d개 구간", len(time_ranges))
    ...
    for idx, (start, end) in enumerate(time_ranges):
        ...
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        logger.info("[편집] 구간 %d: %.1f~%.1fs → %s", idx + 1, start, end, temp_name)
    ...
    logger.info("[편집] 병합 완료: %s", output_path)
