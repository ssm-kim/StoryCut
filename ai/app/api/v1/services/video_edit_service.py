...
def get_video_bitrate(video_path: str) -> Tuple[str, str]:
    ...
    video_bps = probe("v:0") or "3000000"  # default 3Mbps
    audio_bps = probe("a:0") or "128000"
...
async def select_time_ranges_by_prompt(
    ...
    if not filtered_data:
        raise ValueError("❌ threshold 이상의 확률을 가진 데이터가 없습니다.")
    ...
    try:
        ...
        if not isinstance(time_ranges, list):
            raise ValueError("Gemini 응답이 리스트 형식이 아닙니다.")
...
# ✅ FFmpeg로 자르고 병합 (GPU 인코딩, 비트레이트 자동)
